...
<head>
	...
	<script>
		...
		if('${loginFlag}' == 'NO') alert("회원정보를 확인 후 다시 로그인해 주세요.");
	</script>
</head>
<body>
<p><br/></p>
<div class="container">
	...
	<pre>
		...
		이때의 데이터가 주고받는 작업단위를 트랜잭션(Transaction)이라고 한다.
		RequestDispatch 인터페이스 클라이언트의 요청에 의해 컨테이너에 생성된 request와 response를 다른 리소스(서블릿, html, jsp, 이미지)
		로 넘겨주는 역할을 한다.
		이런 역할을 직접 수행하는 메소드가 RequestDispatch 인터페이스의 forward() 메소드이다.
		HTTP 통신의 응답(Response)이 완료되기 전에 서버에 계속 Request 할 수 있는데, 이것은 모두 컨테이너에 담겨 있고,
		forward()에 의해서 response시에 클라이언트에 한번에 전달되게 된다.
	</pre>
	<hr/>
	<form name="myform" method="post" action="<%=request.getContextPath() %>/j0425/T14Ok1">
		...
		<div>보안키
			<input type="password" name="seqKey" value="" class="form-control mb-3" required />
		</div>
		<div>
