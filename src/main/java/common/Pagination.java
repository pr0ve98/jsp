...
public class Pagination {
	...
	public static void pageChange(HttpServletRequest request, int pag, int pageSize, String contentsShow, String section, String part) {
		...
		if(section.equals("board")) {
			...
			// totRecCnt = pdsDao.getTotalRecCnt(contentsShow); // 자료실의 전체/조건에 따른 레코드수 구하기
		}
		...
		if(section.equals("board")) {
			if(part == null || part.equals("")) {
				...
				vos = boardDao.getBoardList(startIndexNo, pageSize, contentsShow, search, searchString); // 게시판에서 검색한 결과 자료 가져오기
			}
		}
		else if(section.equals("pds")) {
